...
def load_raw_dataset(dataset, data_path = None, index_path = None):
    ...
    def select_pos(data, index, index_val):
        final = data[np.where(index == index_val)[0]]
        final = final[final[:, 2] > 3.5]
        return final.astype(np.int32)

    # ret = {
    #     'item_map': item_map,
    #     'train':  select(data, index, 0),
    #     'train_pos':  select_pos(data, index, 0),
    ...
    ret = {
        ...
        'train':  select(data, index, 0),
        'train_pos':  select_pos(data, index, 0),
        'val': select_pos(data, index, 1),
        'test': select_pos(data, index, 2)
    }
    ...
    ret['train_matrix_pos'] = csr_matrix(
        ( np.ones(ret['train_pos'].shape[0]), (ret['train_pos'][:, 0].astype(np.int32), ret['train_pos'][:, 1].astype(np.int32)) ),
        shape = (num_users, num_items)
    )
