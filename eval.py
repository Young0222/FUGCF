...
def evaluate(rating, hyper_params, kernelized_rr_forward, data, item_propensity, train_x, topk = [ 10, 20,], test_set_eval = False):
...
def evaluate_batch(auc_negatives, logits, train_positive, test_positive_set, item_propensity, topk, metrics, train_metrics=False):
    ...
    for b in range(len(logits)):
        if not list(test_positive_set[b]):
            continue
    ...
    print("item_propensity: ", item_propensity)
    print("item_propensity.shape: ", item_propensity.shape)
    for k in topk:
        fairness_sum = 0.0  # Sum of fairness scores for all items
        num_items = 0  # Count of items considered for fairness calculation
        num_users = 0
        item_exposure = {}
        for b in range(len(logits)):
            ...
            if num_pos == 0:
                continue
            num_users += 1
            top_k_items = indices[b][:k]
            for item in top_k_items:
                if item not in item_exposure:
                    item_exposure[item] = 0
                item_exposure[item] += 1
        total_exposures = sum(item_exposure.values())  # Total exposures across all items
        for item, exposure in item_exposure.items():
            ideal_exposure = (item_propensity[item] / item_propensity.sum()) * total_exposures
            if ideal_exposure > 0:
                fairness_sum += ((exposure / ideal_exposure) - 1) ** 2
                num_items += 1
        metrics['EUF@{}'.format(k)] = fairness_sum / (num_items*num_users) if num_items > 0 else 0.0
